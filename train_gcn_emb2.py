...
from utils_model import accuracy, accuracy_mse, RMSELoss
...
def experimenter(data_name='cora', train_ratio=0.03, cuda=True, random_seed=42, hidden=16, dropout_ratio=0.5,
                 learning_rate=0.01, weight_decay=5e-4, num_epochs=100, early_stopping=30, task='classification',
    ...
    with open(file_path, "w") as text_file:
        for i in range(args.replicates):
            ...
            tmp = experimenter(data_name=args.data_name, train_ratio=args.train_ratio, cuda=args.cuda,
                               ...
                               early_stopping=args.early_stopping, task=args.task,
                               public_splitting=args.public_splitting)
            print(tmp)
